window.mostrarInformacoes = function (infoId, detalhesInfo) {
  if (detalhesInfo[infoId]) {
    ...
    historicoNavegacao.push(infoId);
    ...
    if (botaoVoltar) {
      ...
      botaoVoltar.addEventListener('click', function () {
        ...
        if (historicoNavegacao.length > 0) {
          const paginaAnterior = historicoNavegacao[historicoNavegacao.length - 1];
          historicoNavegacao.pop();
          // Mostra a página anterior
          window.mostrarInformacoes(paginaAnterior, detalhesInfo);
        } else {
